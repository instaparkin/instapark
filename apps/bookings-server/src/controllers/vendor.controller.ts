...
export const getVendor = (req: Request, res: Response) => {
	try {
		...
		fetch(
			`https://sandbox.cashfree.com/pg/easy-split/vendors/${userId}`,
			options,
		)
			.then((response) => response.json())
			.then((response) => {
...
export const updateVendor = (req: SessionRequest, res: Response) => {
	try {
		...
		fetch(
			`https://sandbox.cashfree.com/pg/easy-split/vendors/${vendorRequest.vendor_id}`,
			options,
		)
			.then((response) => response.json())
			.then((response) => {
			...
			.catch((error) => {
				sendResponse(
					res,
					500,
					`Error updating payment details: ${error}`,
					'FAILURE',
					null,
				);
			});
...
export const getBalance = (req: SessionRequest, res: Response) => {
	try {
		...
		fetch(
			`https://sandbox.cashfree.com/pg/easy-split/vendors/${vendorId}/balances`,
			options,
		)
			.then((response) => response.json())
			.then((response) => {
