	...
	fields: {
			...
			resolve: async (_, args) => {
				try {
					const bookingResponse = await axios.post<ApiResponse<LockedResponse>>(
						API_SERVER_CONSTANTS.ENDPOINTS.BOOKINGS.BOOKING.LOCK,
						args,
					);
			...
			resolve: async (_, args) => {
				try {
					const response = await axios.post<ApiResponse<null>>(
						API_SERVER_CONSTANTS.ENDPOINTS.BOOKINGS.BOOKING.BOOK,
						args,
					);
		...
		completeOrder: {
			...
			resolve: async (_, args) => {
				try {
					const bookingResponse = await axios.post<ApiResponse<LockedResponse>>(
						API_SERVER_CONSTANTS.ENDPOINTS.BOOKINGS.BOOKING.COMPLETE_ORDER,
						args,
					);
		...
		complete: {
			...
			resolve: async (_, args) => {
				try {
					const response = await axios.post<ApiResponse<null>>(
						API_SERVER_CONSTANTS.ENDPOINTS.BOOKINGS.BOOKING.COMPLETE,
						args,
					);
					return response.data.message;
				} catch (error) {
