				...
				<div className="space-y-4">
					...
					<div className="flex flex-col items-center justify-between gap-4 sm:flex-row">
						...
						<ListingReserveButton
							...
							listingId={listingId}
							basePrice={Number(
								calculator?.items.find((e) => e?.field === 'Base Price')?.value,
							)}
							parkingPrice={Number(
								calculator?.items.find((e) => e?.field?.includes('hours'))
									?.value,
							)}
							totalPrice={Number(
								calculator?.items.find((e) => e?.field === 'Total')?.value,
							)}
							ipFee={Number(
								calculator?.items.find(
									(e) => e?.field === 'Instapark Fee (30%)',
								)?.value,
							)}
						/>
					</div>
