...
export const autoCompleteLocations = createAsyncThunk<
	...
	string,
	ThunkApiConfig
>('maps/autocomplete', async (query, { rejectWithValue }) => {
	try {
...
export const directions = createAsyncThunk<
	MapData[],
	{ origin: number[]; destination: number[] },
	ThunkApiConfig
>('maps/directions', async ({ origin, destination }, { rejectWithValue }) => {
	try {
...
export const reverseGeocodeLocation = createAsyncThunk<
	...
	number[],
	ThunkApiConfig
>('maps/reverse-geocode', async (latlng, { rejectWithValue }) => {
	try {
...
export const geocodeLocation = createAsyncThunk<
	...
	string,
	ThunkApiConfig
>('maps/geocode', async (q, { rejectWithValue }) => {
	try {
		const response = await axios.get(
...
export const mapsSlice = createSlice({
	...
	extraReducers: (builder) => {
		builder
			...
			.addCase(
				autoCompleteLocations.fulfilled,
			...
			.addCase(
				autoCompleteLocations.rejected,
			...
			.addCase(
				reverseGeocodeLocation.fulfilled,
				(state, action: PayloadAction<MapData[]>) => {
					state.status = 'succeeded';
			...
			.addCase(
				reverseGeocodeLocation.rejected,
				(state, action: PayloadAction<string | undefined>) => {
					state.status = 'failed';
					state.error = action.payload || 'Unknown error occurred';
				},
